...
def createCentroids(k, datadict):
    '''(int, dict) -> list
    Finds random center points, or centroids, for the number of clusters wanted
    ...
    while centroidCount < k:
        rkey = random.randint(1, len(datadict)) #randomizes centroid
...
def createClusters(k, centroids, datadict, repeats):
    ...
    Creates clusters of earthquakes using the earthquakes' dictionary keys
    Returns a list of clusters containing a list of which earthquakes are in the cluster
    ...
    '''
    for apass in range(repeats): #more repeats -> more precise clusters
        #print('**** PASS', apass, '****')
        clusters = []
        for i in range(k): #adds empty lists for the number of clusters wanted
            clusters.append([])
        for akey in datadict: #assigns each earthquake to a cluster
            distances = []
        ...
        for clusterIndex in range(k): #re-determine the centroids
            sums = [0] * dimensions
...
#print(doctest.testmod()) #Doesn't work with functions relying on random or using turtle
