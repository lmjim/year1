...
def solve(board: Board) -> bool:
    ...
    if not board.is_consistent():
        return False
    min_candidates = 10
    for groups in board.groups:
        for tile in groups.tiles:
            if 1 < len(tile.candidates) < min_candidates:
                min_candidates = len(tile.candidates)
                guess_tile = tile
    before = board.as_list()
    for option in guess_tile.candidates:
        guess_tile.set_value(option)
        if solve(board):
            return True
        else:
            board.set_tiles(before)