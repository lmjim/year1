...
class Checker(object):
    ...
    def __init__(self, level):
        """Initialize to threshold level of some quality"""
        self.level = level
...
class CrunchChecker(Checker):
    """Checks for crunchiness above threshold"""
    def check(self, obj) -> bool:
        return obj.crunch >= self.level


class SweetChecker(Checker):
    """ Checks for sweetness above threshold"""
    def check(self, obj) -> bool:
...
class FruitBowl(object):
    ...
    def choose(self, checker: Checker):
        ...
        for fruit in self.contents:
            if checker.check(fruit):  # if fruit.check(checker):
